...
def normalize_upload_filename(raw_filename: str) -> str:
    """Normalize upload filename without stripping Unicode characters."""
    if raw_filename is None:
        raise HTTPException(status_code=400, detail='文件名为空')

    filename = raw_filename.replace('\x00', '')
...
async def chat_with_context(request: ChatWithContextRequest):
    ...
    try:
        ...
        # 4. Agent 已经通过 ChatMemoryBuffer 写入会话历史，这里仅同步持久化/元数据
        chat_store_service.sync_session(session_id)
...
async def get_chat_history(session_id: str, limit: int = Query(50, ge=1, le=100)):
    ...
    try:
        ...
        logger.debug(f"session id：{session_id},get_chat_history接口返回值：{history}")
